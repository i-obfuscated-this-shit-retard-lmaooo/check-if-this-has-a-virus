...
bool c_gui::is_key_enabled(int* key, int* mode) {
    if (!key || !mode || !(*key)) return false;
    if (*mode == 2) return true; // always
    switch (*mode) {
    case 0: return GetAsyncKeyState(*key) & 0x8000;
    case 1: {
        auto& state = key_states[key];
        if (GetAsyncKeyState(*key) & 0x1) state = !state;
        return state;
    }
    }
    return false;
