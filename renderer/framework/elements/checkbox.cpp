...
bool c_gui::checkbox(std::string_view label, std::atomic<bool>* callback) {
    auto yes = callback->load(std::memory_order_relaxed);
    auto pressed = checkbox(label, &yes);
    if (pressed) callback->store(yes, std::memory_order_relaxed);
    return pressed;
}