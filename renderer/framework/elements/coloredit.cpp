...
// custom
bool c_gui::color_edit(std::string_view label, ImColor& col, bool active_alpha)
{
    float color[4] = { col.Value.x, col.Value.y, col.Value.z, col.Value.w };
    auto value_changed = color_edit(label, color, active_alpha);
    if (value_changed) { col.Value.x = color[0]; col.Value.y = color[1]; col.Value.z = color[2]; col.Value.w = color[3]; }
    return value_changed;
}
...
bool c_gui::label_color_edit(std::string_view label, ImColor& col, bool active_alpha)
{
    //draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text), label.data());
    draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), draw->get_clr(clr->widgets.text), label.data());
    gui->set_cursor_pos(ImVec2(GetWindowWidth() - elements->widgets.color_size.x - GetStyle().WindowPadding.x, GetCursorPos().y));
