...
bool dd_selectable_ex(const char* label, bool active)
{
    ...
    // custom fix
    const ImRect rect(pos, pos + ImVec2(width, g.FontSize + g.Style.FramePadding.y * 2.f)); // previously y was 15
    //const ImRect rect(pos, pos + ImVec2(width, 20)); // try 2
    //const ImRect rect(pos, pos + ImVec2(width, 15)); // original
    ...
    // custom fix
    const float font_size = var->font.tahoma->FontSize;
    ImVec2 text_pos = rect.Min + ImVec2(10, ((g.FontSize + g.Style.FramePadding.y * 2.f) - font_size) * 0.5f); // previously y was -1
    //ImVec2 text_pos = rect.Min + ImVec2(10, (20 - font_size) * 0.5f); // try 2
    //ImVec2 text_pos = rect.Min + ImVec2(10, -1); // original

    draw->text_clipped_outline(window->DrawList, var->font.tahoma, text_pos, rect.Max - ImVec2(0, 1), draw->get_clr(active && !hovered ? clr->accent : hovered ? clr->widgets.text_inactive : clr->widgets.text), label, NULL, NULL, ImVec2(0.f, 0.5f));
...
bool begin_dropdown_popup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags, int val)
{
    ...
    else
    {
        ...
        SetNextWindowSize(ImVec2(constraint_min.x, ((g.FontSize + g.Style.FramePadding.y * 2.f) * val + 3 * (val - 1)) + 10));
        //SetNextWindowSize(ImVec2(constraint_min.x, ((g.FontSize + g.Style.FramePadding.y * 2.f) * val) + g.Style.ItemSpacing.y * (val - 1) + g.Style.WindowPadding.y));
        //SetNextWindowSize(ImVec2(constraint_min.x, (20 * val + 3 * (val - 1)) + 5)); // try 2
    }
...
bool begin_dropdown(const char* label, const char* preview_value, bool wo_label, int val, ImGuiComboFlags flags)
{
    ...
    if (!wo_label) draw->text_clipped_outline(window->DrawList, var->font.tahoma, rect.Min - ImVec2(-1, 2), ImVec2(rect.Max.x, clickable.Min.y - 2), draw->get_clr(clr->widgets.text), label, NULL, NULL, ImVec2(0.f, 0.f));
...
void c_gui::multi_dropdown(std::string_view label, bool variables[], const char* labels[], int count, bool wo_label)
{
    std::string preview = "-";
    ...
    if (begin_dropdown(label.data(), preview.c_str(), wo_label, count, 0))
    {
        for (auto i = 0; i < count; i++)
        {
            dd_selectable(labels[i], &variables[i]);
        }
        end_dropdown();
    }
