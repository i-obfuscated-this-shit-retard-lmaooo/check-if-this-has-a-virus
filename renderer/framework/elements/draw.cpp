...
{
    if (!text_end) text_end = FindRenderedTextEnd(text_begin);
...
void c_draw::text_clipped(ImDrawList* draw_list, ImFont* font, float font_size, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    PushFont(font);
    ...
    {
        ImVec4 fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
        draw_list->AddText(NULL, font_size, pos, color, text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw_list->AddText(NULL, font_size, pos, color, text, text_display_end, 0.0f, NULL);
    }
...
{
    if (!text_display_end) text_display_end = FindRenderedTextEnd(text);
...
void c_draw::text_clipped_outline(ImDrawList* draw_list, ImFont* font, float font_size, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    if (!text_display_end) text_display_end = FindRenderedTextEnd(text);
    text_clipped(draw_list, font, font_size, pos_min - ImVec2(1, 1), pos_max - ImVec2(1, 1), draw->get_clr({ 0, 0, 0, 1 }), text, text_display_end, text_size_if_known, align);
    text_clipped(draw_list, font, font_size, pos_min - ImVec2(1, -1), pos_max - ImVec2(1, -1), draw->get_clr({ 0, 0, 0, 1 }), text, text_display_end, text_size_if_known, align);
    text_clipped(draw_list, font, font_size, pos_min + ImVec2(1, 1), pos_max + ImVec2(1, 1), draw->get_clr({ 0, 0, 0, 1 }), text, text_display_end, text_size_if_known, align);
    text_clipped(draw_list, font, font_size, pos_min + ImVec2(1, -1), pos_max + ImVec2(1, -1), draw->get_clr({ 0, 0, 0, 1 }), text, text_display_end, text_size_if_known, align);
    text_clipped(draw_list, font, font_size, pos_min, pos_max, color, text, text_display_end, text_size_if_known, align);
}
...
// custom
void c_draw::window_decorations(bool icon = false, ImTextureID texture = 0)
{
    draw->rect(GetBackgroundDrawList(), GetWindowPos() - ImVec2(1, 1), GetWindowPos() + GetWindowSize() + ImVec2(1, 1), draw->get_clr({0, 0, 0, 0.5f}));
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(0, var->window.titlebar), GetWindowPos() + GetWindowSize(), draw->get_clr(clr->window.background_two));
    draw->fade_rect_filled(GetWindowDrawList(), GetWindowPos(), GetWindowPos() + ImVec2(GetWindowWidth(), var->window.titlebar), draw->get_clr(clr->window.background_one), draw->get_clr(clr->window.background_two), fade_direction::vertically);
    ...
    draw->rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(6, var->window.titlebar), GetWindowPos() + GetWindowSize() - ImVec2(6, 6), draw->get_clr(clr->window.background_one));
    draw->rect(GetWindowDrawList(), GetWindowPos() + ImVec2(6, var->window.titlebar), GetWindowPos() + GetWindowSize() - ImVec2(6, 6), draw->get_clr(clr->window.stroke));
    if (icon) draw->image(GetWindowDrawList(), texture, GetWindowPos() + ImVec2{ var->window.spacing.x, 2 }, GetWindowPos() + ImVec2{ var->window.spacing.x + 18, 20 });
    draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, icon ? GetWindowPos() + ImVec2(23, 4) : GetWindowPos() + ImVec2(7, 4), draw->get_clr(clr->accent), GetCurrentWindow()->Name);
}